...
class ReviewsService:
    ...
    def __init__(self, reviews_repository: ReviewsRepository, books_service: BooksService, users_service: UsersService):
    ...
    async def create(self, review: BaseReviewSchema) -> Review:
        await asyncio.gather(self.books_service.get_one_without_rating(review.book_id),
    ...
    async def query(self, filter_attributes: list[ReviewFilterEnum] = None, filter_values: list[str] = None,
                    sort: ReviewFilterEnum = None, sort_direction: SortEnum = None,
                    page: int = None, size: int = None) -> (list[Review], int):
        filters_dict = {}
        if filter_attributes and filter_values:
            ...
            for attribute in filter_attributes:
                ...
                i += 1
        return await self.__reviews_repository.query(filters_dict=filters_dict,
    ...
    async def __get_review_by_id_if_exists(self, review_id: ObjectId) -> Review:
        ...
        if not review:
            raise ObjectNotFoundException(detail='Review with id ' + str(review_id) + ' not found')
        return review
