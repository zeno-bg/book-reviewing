...
async def update(book_id: ObjectId, book_new: BookPatchSchema, books_service: BooksServiceDep) -> Book:
...
@router.get('/')
async def query(books_service: BooksServiceDep,
    ...
    items, total_count = await books_service.query(filter_attributes, filter_values, sort, sort_direction, page, size)
