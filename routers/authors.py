...
@router.post("/")
async def create(
    author: BaseAuthorSchema, authors_service: AuthorsServiceDep
) -> Author:
...
async def update(
    author_id: ObjectId,
    author_new: AuthorPatchSchema,
    authors_service: AuthorsServiceDep,
) -> Author:
...
async def get_one(
    author_id: ObjectId, authors_service: AuthorsServiceDep
) -> AuthorOutSchema:
...
async def query(
    ...
    items, total_count = await authors_service.query(
...
@router.delete(
    "/{author_id}",
    status_code=204,
    description="Also deletes all books for this author!",
)
