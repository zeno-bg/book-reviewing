...
@router.post('/')
async def create(review: BaseReviewSchema, reviews_service: ReviewsServiceDep) -> Review:
    return await reviews_service.create(review)


@router.patch('/{review_id}')
async def update(review_id: ObjectId, review_new: ReviewPatchSchema, reviews_service: ReviewsServiceDep) -> Review:
...
@router.get('/')
async def query(reviews_service: ReviewsServiceDep,
    ...
    items, total_count = await reviews_service.query(filter_attributes, filter_values, sort, sort_direction, page, size)
